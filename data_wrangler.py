...
class DataWrangler(object):
    ...
    def __init__(self, image_size=config.IMAGE_SIZE, batch_size=config.BATCH_SIZE, train_path=config.TRAIN_DIR, val_path=config.VAL_DIR, classes=None):
...
class CustomDataWrangler(DataWrangler):
    ...
    def get_train_generator(self):
        with open("data\\training.csv") as f: #clean
            data = f.read().split("\n")
        files = []
        data.pop()
        for line in data:
    ...
    def get_val_generator(self):
        with open("data\\validation.csv") as f: #clean
            data = f.read().split("\n")
        data.pop()
        files = []
        for line in data:
...
class E2KGenerator(object):
    def __init__(self, element_classifier, image_generator):
        self.image_generator = image_generator
        self.cnn = element_classifier
    ...
    def __next__(self):
        images, labels = self.image_generator.__next__()
        print(images.shape, labels.shape)
        predictions = self.cnn.predict(images)
        return (predictions, labels)


class E2KDataWrangler(DataWrangler):
